...
import { Plus, Edit3, Trash2, Copy, CheckCircle2, XCircle, Cpu } from 'lucide-react'
...
const ARCHITECTURES: Architecture[] = [
  {
    ...
    usageCount: 47,
    params: [
      { name: '模型尺寸', key: 'variant', type: 'select', defaultValue: 'm', options: [
        { label: 'YOLOv8n (Nano)', value: 'n' },
        { label: 'YOLOv8s (Small)', value: 's' },
        { label: 'YOLOv8m (Medium)', value: 'm' },
        { label: 'YOLOv8l (Large)', value: 'l' },
        { label: 'YOLOv8x (XLarge)', value: 'x' },
      ], required: true, description: '模型尺寸变体' },
      ...
      { name: '优化器', key: 'optimizer', type: 'select', defaultValue: 'SGD', options: [
        { label: 'SGD（推荐）', value: 'SGD' },
        { label: 'Adam', value: 'Adam' },
        { label: 'AdamW', value: 'AdamW' },
        { label: 'RMSProp', value: 'RMSProp' },
      ], required: true, description: '优化器类型' },
      ...
      { name: '数据增强策略', key: 'augment', type: 'select', defaultValue: 'default', options: [
  ...
  },
  {
    ...
    usageCount: 23,
    params: [
      { name: '模型尺寸', key: 'variant', type: 'select', defaultValue: 's', options: [
        { label: 'YOLOv8n (Nano)', value: 'n' },
        { label: 'YOLOv8s (Small)', value: 's' },
        { label: 'YOLOv8m (Medium)', value: 'm' },
      ], required: true, description: '模型尺寸变体' },
      ...
      { name: '优化器', key: 'optimizer', type: 'select', defaultValue: 'SGD', options: [
        { label: 'SGD', value: 'SGD' },
        { label: 'Adam', value: 'Adam' },
      ], required: false, description: '优化器类型' },
    ],
  },
  {
    id: 'arch-qwen',
    ...
    usageCount: 18,
    params: [
      { name: '模型版本', key: 'baseModel', type: 'select', defaultValue: 'Qwen-7B-Chat', options: [
      ...
      ], required: true, description: '预训练模型版本' },
      { name: '微调方式', key: 'finetuneMode', type: 'select', defaultValue: 'lora', options: [
        { label: 'LoRA（参数高效）', value: 'lora' },
        { label: '全参数微调', value: 'full' },
      ], required: true, description: '微调方式' },
      ...
      { name: '目标层', key: 'targetLayers', type: 'select', defaultValue: 'q_proj,v_proj', options: [
        { label: '仅 Q/V', value: 'q_proj,v_proj' },
  ...
  },
  {
    ...
    usageCount: 8,
    params: [
      { name: '模型版本', key: 'baseModel', type: 'select', defaultValue: 'LLaMA-2-7B-Chat', options: [
    ...
    <div style={{ animation: 'slideIn 0.3s ease-out' }}>
      ...
      <div className="content-padded">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 16 }}>
          {filtered.map(arch => (
            <div key={arch.id} className="card" style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{arch.name}</div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                ...
                <span className="badge badge-secondary">{arch.params.length} 个参数</span>
                <span className="badge badge-secondary">{arch.usageCount} 个预设引用</span>
              </div>

              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 12 }}>
                {arch.author} · {arch.createdAt}
              </div>

              <div style={{ display: 'flex', gap: 6 }}>
                <Link
                  to="/architectures/$architectureId"
                  params={{ architectureId: arch.id }}
