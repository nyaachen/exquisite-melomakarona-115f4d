...
function EditTemplate() {
  ...
  const templateData = TEMPLATES.find(t => t.id === templateId) || TEMPLATES[0]
  
  const [name, setName] = useState(templateData.name)
  const [modelType, setModelType] = useState(templateData.modelType)
  const [baseModel, setBaseModel] = useState(templateData.baseModel)
  const [description, setDescription] = useState(templateData.description)
  const [isActive, setIsActive] = useState(templateData.isActive)
  const [params, setParams] = useState<TemplateParam[]>(templateData.params)
    ...
    <div style={{ animation: 'slideIn 0.3s ease-out', padding: '24px 32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          ...
          <h1 className="page-title">编辑训练模板</h1>
        </div>
      </div>
      ...
      <div className="card" style={{ maxWidth: 800, margin: '0 auto' }}>
        <div style={{ padding: 24 }}>
          {templateData.usageCount > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', background: 'rgba(245,158,11,0.08)', borderRadius: 8, border: '1px solid rgba(245,158,11,0.15)', marginBottom: 20 }}>
              <AlertCircle size={16} style={{ color: 'var(--warning)', flexShrink: 0 }} />
              <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                该模板已被使用 <strong style={{ color: 'var(--warning)' }}>{templateData.usageCount}</strong> 次，修改后将影响后续训练任务
              </span>
            </div>
          )}
