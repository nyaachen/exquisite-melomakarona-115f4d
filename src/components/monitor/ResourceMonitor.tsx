...
function ServerCard({ server, now }: { server: GpuServer; now: number }) {
  ...
  return (
    <div className="card" style={{ overflow: 'hidden' }}>
      ...
      {/* Per-GPU rows: card info + task info horizontally */}
      {server.gpus.map((g, i) => {
...
export function ResourceMonitor() {
  ...
  const busyGpuCount = GPU_SERVERS.flatMap(s => s.gpus).filter(g => g.assignedTask).length
  const totalGpuCount = GPU_SERVERS.flatMap(s => s.gpus).length

  return (
    <div>
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
        {onlineCount} 台在线 · {busyGpuCount}/{totalGpuCount} 卡运行中
      </div>
